...
import type { IMessageAddress, ProviderMessage, ProviderSentMessage } from '../src/index.js';
import {
  ClientClosedError,
...
function createProviderMessage(overrides: Partial<ProviderMessage> = {}): ProviderSentMessage {
  return {
...
describe('generic client', () => {
  ...
  it('preserves the selected provider and connection literals', async () => {
    ...
    expect(client.providers.blooio.capabilities).toBe(TEST_CAPABILITIES);
  });
  ...
  it('closes idempotently and rejects later operations', async () => {
    ...
    await expect(client.messages.send({ to: recipient, text: 'Too late' })).rejects.toBeInstanceOf(
      ClientClosedError,
    );
  });
});
