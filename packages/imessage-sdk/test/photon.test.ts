...
vi.mock("@photon-ai/advanced-imessage", async (importOriginal) => {
  const actual =
    await importOriginal<typeof import("@photon-ai/advanced-imessage")>();
...
const chat: Chat = {
  ...
  isGroup: false,
  participants: [
    { address: recipientPhone, service: "iMessage" },
  ],
  service: "iMessage",
};
  ...
  it("catches up before draining live events and deduplicates sequences", async () => {
    ...
    sdk.catchUp.mockReturnValue(
      stream<CatchUpEvent>([
        received,
        reaction,
        { type: "catchup.complete", headSequence: 8 },
      ]),
    );
